...
{
	spinlock_t *lock = NULL;
	//无锁qdisc 加锁，注意这里这个是阻塞等待
	//这里的无锁是指不需要root 和 busy lock
	if (q->flags & TCQ_F_NOLOCK) {
		lock = qdisc_lock(q);
		spin_lock(lock);
	}
...
validate:
	*validate = true;
	//一个队列一个qdisc的情况，如果队列已经freez
	if ((q->flags & TCQ_F_ONETXQUEUE) &&
	    netif_xmit_frozen_or_stopped(txq)) {
		qdisc_maybe_clear_missed(q, txq);
		return skb;
	}
	//从bad txq 取这里面队列不同的数据包，入队的地方就在下面
	...
	skb = q->dequeue(q);
	if (skb) {
bulk:	//一个队列一个qdisc的情况，在多取几个 
		if (qdisc_may_bulk(q))
			try_bulk_dequeue_skb(q, skb, txq, packets);
		else
		//一个qdisc可能有其他硬件队列中的数据包，如果不同就放到bad txq中最多取8个
		     ...
		     spinlock_t *root_lock, bool validate)
{
	...
	/* And release qdisc */
	//释放了root lock 因为要发包比较耗时
	if (root_lock)
		spin_unlock(root_lock);

	/* Note that we validate skb (GSO, checksum, ...) outside of locks */
	if (validate)
		//GSO vlan 分散聚合等硬件卸载能力校验
	...
	if (likely(skb)) {
		...
		if (!netif_xmit_frozen_or_stopped(txq))
			...
			qdisc_maybe_clear_missed(q, txq);
		//给网卡发送队列加锁 保证ndo_start_xmit不被并发调用
...
void __qdisc_run(struct Qdisc *q)
{
	...
	while (qdisc_restart(q, &packets)) {
		quota -= packets;
		//配额用尽的情况下调度软中断发包，无锁情况也是一样的，会因为设置标志位调度
		if (quota <= 0) {
			if (q->flags & TCQ_F_NOLOCK)
				set_bit(__QDISC_STATE_MISSED, &q->state);
			else
...
static int pfifo_fast_enqueue(struct sk_buff *skb, struct Qdisc *qdisc,
			      struct sk_buff **to_free)
{
	int band = prio2band[skb->priority & TC_PRIO_MAX]; //获取优先级
	struct pfifo_fast_priv *priv = qdisc_priv(qdisc); //拿到私有数据，也就是队列
	struct skb_array *q = band2list(priv, band); //找到所属的队列
	unsigned int pkt_len = qdisc_pkt_len(skb); //获取数据包的长度
	int err;
	//数据包入队
...
static int pfifo_fast_init(struct Qdisc *qdisc, struct nlattr *opt,
			   struct netlink_ext_ack *extack)
{
	unsigned int qlen = qdisc_dev(qdisc)->tx_queue_len; //获取设备队列的长度
	struct pfifo_fast_priv *priv = qdisc_priv(qdisc); //pfifofast的私有数据
	int prio;
...
struct Qdisc_ops pfifo_fast_ops __read_mostly = {
	...
	.peek		=	pfifo_fast_peek,	//peek
	.init		=	pfifo_fast_init,	//初始化
	.destroy	=	pfifo_fast_destroy,  //释放资源
	.reset		=	pfifo_fast_reset,
	.dump		=	pfifo_fast_dump,
	.change_tx_queue_len =  pfifo_fast_change_tx_queue_len, //
	.owner		=	THIS_MODULE,
	.static_flags	=	TCQ_F_NOLOCK | TCQ_F_CPUSTATS,  //唯一无锁的标志
};
				...
				struct netlink_ext_ack *extack)
{
	...
	//设置 parent 某个class或者qdisc
	sch->parent = parentid;
	//调用qdisc的init，初始化阶段调用的就是mq的，然后返回
...
static void attach_default_qdiscs(struct net_device *dev)
{
	...
	if (!netif_is_multiqueue(dev) ||
		...
		//把设备级 dev->qdisc 指向第 0 个队列的 sleeping qdis，因为是单队列
...
//如果当前这个 qdisc 不是 noop_qdisc，就把它加入 qdisc 哈希表。
#ifdef CONFIG_NET_SCHED
	if (qdisc != &noop_qdisc)
...
void dev_init_scheduler(struct net_device *dev)
{
	...
	//遍历网卡的每个队列，这里的队列数量是申请网络设备时候驱动注册的
	netdev_for_each_tx_queue(dev, dev_init_scheduler_queue, &noop_qdisc);
	//如果这个设备存在ingress 也需要初始化，这里大概率没有
	if (dev_ingress_queue(dev))
		dev_init_scheduler_queue(dev, dev_ingress_queue(dev), &noop_qdisc);
	//这里设置watchdog 用于判断发送队列是不是卡死了 一直发不出去
