...
struct ip_vs_conn *
//根据ip和port查找入方向的连接 ct 其实就是hash一下比较key
...
//这里注意，nat模式下，在实服务器回给客户端的时候这里是可以找到的
...
int ip_vs_check_template(struct ip_vs_conn *ct, struct ip_vs_dest *cdest)
{
	 ...
	 */
	if ((dest == NULL) ||      //已经被删除？
	    !(dest->flags & IP_VS_DEST_F_AVAILABLE) || //update_dest 会设置
	    expire_quiescent_template(ipvs, dest) || //静默状态
	    (cdest && (dest != cdest))) {   //不匹配
	       ...
	       struct ip_vs_dest *dest, __u32 fwmark)
{
	...
	//保存上下文与协议族：cp->ipvs/ip->af/daf/protocol
	cp->ipvs	   = ipvs;
	cp->af		   = p->af;
	cp->daf		   = dest_af;
	...
	//加入到hash 表中 （大概率是客户端的ip和port用做hash来插入hash表）
	ip_vs_conn_hash(cp);
