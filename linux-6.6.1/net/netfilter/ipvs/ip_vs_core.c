		    ...
		    int *ignored, struct ip_vs_iphdr *iph)
{
	...
	//创建连接模板参数
	{
		...
		//把协议 ip port 等保存起来 注意这里的客户端port是0
	...
	if (!ct || !ip_vs_check_template(ct, NULL)) {
		...
		// 若服务端口匹配且不是 FTP，把模板上的 dport 设成后端端口
 ...
 */
//ipvs进入local hook点的地一个钩子，用于处理服务端回给客户端的数据包
static unsigned int
ip_vs_out_hook(void *priv, struct sk_buff *skb, const struct nf_hook_state *state)
{
	 ...
	 */
	//这里注意，如果是客户端发往服务端的数据，这里是找到不的
	cp = INDIRECT_CALL_1(pp->conn_out_get, ip_vs_conn_out_get_proto,
			     ipvs, af, skb, &iph);
	//如果是服务端发往客户端的数据，从这里直接就返回了
	if (likely(cp))
	...
	if (sysctl_nat_icmp_send(ipvs) &&
		...
		if (ip_vs_has_real_service(ipvs, af, iph.protocol, &iph.saddr,
			 ...
			 * existing entry if it is not RS T
...
{
	struct ip_vs_protocol *pp = pd->pp;
	//这里为什么判断是否是分片呢，ipvs那到input hook点不应该已经是重组完成的吗 针对ipv6的处理？
	if (!iph->fragoffs) {
...
ip_vs_in_hook(void *priv, struct sk_buff *skb, const struct nf_hook_state *state)
{
	 ...
	 */
	//不是发往本机的数据包，且不是localout hook点，或没有路由信息,直接放行
	...
	if (unlikely(sysctl_backup_only(ipvs) || !ipvs->enable))//register hook 会enable
		return NF_ACCEPT;
	//填充ip头
	ip_vs_fill_iph_skb(af, skb, false, &iph);
	...
	cp = INDIRECT_CALL_1(pp->conn_in_get, ip_vs_conn_in_get_proto,
			     ipvs, af, skb, &iph);
	//不是分片报文，是新创建的链接， 这里为什么会判断是否分片呢？ 这里肯定不会是分片报文啊
	//这里的目的是判断是否在下面重新选择一个实服务器
	if (!iph.fragoffs && is_new_conn(skb, &iph) && cp) {
		int conn_reuse_mode = sysctl_conn_reuse_mode(ipvs);
		bool old_ct = false, resched = false;

		if (unlikely(sysctl_expire_nodest_conn(ipvs)) && cp->dest && //实服务器不可以用时重新调度选项
		    unlikely(!atomic_read(&cp->dest->weight))) {   //实服务器权重为0
			resched = true; //需要重新调度
			old_ct = ip_vs_conn_uses_old_conntrack(cp, skb);  //false
		} else if (conn_reuse_mode &&
			   is_new_conn_expected(cp, conn_reuse_mode)) {  //例如tcp的syn包这里返回的是false
			old_ct = ip_vs_conn_uses_old_conntrack(cp, skb); //false
		...
		if (resched) {
			...
			if (!atomic_read(&cp->n_control))
				ip_vs_conn_expire_now(cp);//立即让定时器过期
			__ip_vs_conn_put(cp);
			if (old_ct)
				return NF_DROP;
			cp = NULL;
	...
	if (cp && cp->dest && !(cp->dest->flags & IP_VS_DEST_F_AVAILABLE)) {
		...
		if (sysctl_expire_nodest_conn(ipvs)) {
			bool old_ct = ip_vs_conn_uses_old_conntrack(cp, skb); //没有ct的话返回false

			if (!old_ct)
				cp->flags &= ~IP_VS_CONN_F_NFCT;

			ip_vs_conn_expire_now(cp);//清除ct
			__ip_vs_conn_put(cp);
			if (old_ct)
				return NF_DROP;
			cp = NULL;
