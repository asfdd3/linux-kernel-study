		  ...
		  struct ip_vs_iphdr *iph)
{
	...
	if (likely(!ip_vs_iph_icmp(iph))) {
		...
		if (th) {
			//只有syn包继续处理！
			if (th->rst || !(sysctl_sloppy_tcp(ipvs) || th->syn))//不是syn包
				return 1;
			//提取源port
			ports = &th->source;
		}
	...
	if (likely(!ip_vs_iph_inverse(iph)))
		...
		svc = ip_vs_service_find(ipvs, af, skb->mark, iph->protocol,
					 &iph->saddr, ports[0]);
	//找到了虚服务，正常都是可以找到的吧，这里注意如果没要找到配置的虚服务是会放行数据包的！
	if (svc) {
		int ignored;
		//是否需要丢包  一种防御性机制？  这个目的是什么呢？
		 ...
		 struct ip_vs_conn *cp, struct ip_vs_iphdr *iph)
{
	...
	if (cp->af == AF_INET6 && iph->fragoffs)
		return 1;
#endif
	oldlen = skb->len - tcphoff; //这里oldlen是tcp头家负载的长度
	      ...
	      int direction, struct tcphdr *th)
{
	...
	new_state =
		pd->tcp_state_table[state_off+state_idx].next_state[cp->state];//更新状态
...
{
	//这个叫初始化应用助手hash表，好像用来解析特定协议的时候用到
...
struct ip_vs_protocol ip_vs_protocol_tcp = {
	...
	.init =			NULL,
	.exit =			NULL,
	.init_netns =		__ip_vs_tcp_init,//初始化apphash表 超时时间表 //ip_vs_init中会调用
	.exit_netns =		__ip_vs_tcp_exit,
	.register_app =		tcp_register_app,  // 什么时候会调用这个？ 例如moduleinit加载FTP应用助手模块的时候会调用
	.unregister_app =	tcp_unregister_app,
	.conn_schedule =	tcp_conn_schedule,  //比如新的TCP连接到达，就会调用这个
	.conn_in_get =		ip_vs_conn_in_get_proto,   //hash一下 比较key 找到ct
	.conn_out_get =		ip_vs_conn_out_get_proto,  //同上
	.snat_handler =		tcp_snat_handler,      	//RS -> client 的时候会调用
	.dnat_handler =		tcp_dnat_handler,		//client -> RS 的时候会调用
	.state_name =		tcp_state_name,			//展示当前TCP的状态
