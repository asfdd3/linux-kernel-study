...
struct file *sock_alloc_file(struct socket *sock, int flags, const char *dname)
{
	...
	//alloc一个file绑定了socket_file_ops 回调函数集合 sock_mnt为一个挂在点
...
{	
	...
	//根据fd从进程管理的fdtable中找到对应的file，这里struct fd结构体中的一个字段为file
...
struct socket *sock_alloc(void)
{
	struct inode *inode;
	struct socket *sock;
	//调用socket文件系统的超级块的ops申请一个inode，注意：socket结构体也是在这里分配的
	inode = new_inode_pseudo(sock_mnt->mnt_sb);
	if (!inode)
		return NULL;
	//通过container_of拿到socket结构体 
	sock = SOCKET_I(inode);

	inode->i_ino = get_next_ino();//分配唯一的inode编号
	inode->i_mode = S_IFSOCK | S_IRWXUGO; //文件类型
			 ...
			 struct socket **res, int kern)
{
	 ...
	 */
	//过时的 PF_INET + SOCK_PACKET 参数组合转换为现代支持的 PF_PACKET 协议族
	 ...
	 *	default.
	 */
	//分配并初始化一个套接字对应的 inode 和socket 结构
	sock = sock_alloc();
	if (!sock) {
	...
	rcu_read_lock();
	//从sock_register数组中找到一个元素pf，这个pf中有一个create()回调函数，
	//这个回调函数就是family类型(比如AF_INET)需要的create函数。
	 ...
	 */
	//增加引用计数？有些family可能是以模块方式加载的？？？
	if (!try_module_get(pf->owner))
		goto out_release;

	/* Now protected by module ref count */
	rcu_read_unlock();
	//如果用户指定的family类型是AF_INIT,那这个函数就是调用的inet_create()
...
int __sys_bind(int fd, struct sockaddr __user *umyaddr, int addrlen)
{
	...
	struct sockaddr_storage address;
	int err, fput_needed;
	//通过fd 查找到file，通过file返回私有指针，这个私有指针就是socket
 ...
 *	ready for listening.
 */
//这里的backlog表示已经完成三次握手但是没有被accept的最大数量
		       ...
		       int __user *upeer_addrlen, int flags)
{
	...
	struct sockaddr_storage address;
	const struct proto_ops *ops;
	//通过accpet 传入的listensock的私有结构中拿到socket
	sock = sock_from_file(file);
	if (!sock)
	...
	//创建一个file结构体(也就是把上面申请的socket的结构提挂到file的私有结构上)
	...
	if (err)
		goto out_fd;
	//调用套接子的accept回调，对于tcp就是inet_accept
			      ...
			      int __user *upeer_addrlen, int flags)
{
	...
	if (SOCK_NONBLOCK != O_NONBLOCK && (flags & SOCK_NONBLOCK))
		flags = (flags & ~SOCK_NONBLOCK) | O_NONBLOCK;
	//这里先获取一个没有使用的fd，下面会跟一个文件相关联
	newfd = get_unused_fd_flags(flags);
	if (unlikely(newfd < 0))
		return newfd;
	//真正的accpet，这里返回了一个file，传入的file为用户fd关联的file
		       ...
		       int addrlen, int file_flags)
{
	...
	if (err)
		goto out;
	//调用套接字层 connect __inet_stream_connect
