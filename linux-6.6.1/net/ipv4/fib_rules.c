		 ...
		 struct fib_result *res, unsigned int flags)
{
	...
	//拿着当前这个 flow，去遍历 IPv4 的 rule 链，按 rule 去匹配
	//这里注意，协议栈初始化的时候会注册三个表！！！main local default 通过ip命令也可以添加
...
static const struct fib_rules_ops __net_initconst fib4_rules_ops_template = {
	...
	.action		= fib4_rule_action,		//策略路由
	.suppress	= fib4_rule_suppress,	//策略路由
	.match		= fib4_rule_match,    //数据包和规则是否配，只有策略路由才调用！
	.configure	= fib4_rule_configure,	//把 netlink 传进来的 rule 参数解析 ip rule add
	.delete		= fib4_rule_delete,		//ip rule del
	.compare	= fib4_rule_compare,
	.fill		= fib4_rule_fill, 		 //ip rule show
	.nlmsg_payload	= fib4_rule_nlmsg_payload, // 构造 netlink 消息时
...
{
	int err;
	//local main default 创建规则（查表顺序），然后挂到ops中
...
//初始化“查路由的顺序规则” 注意ops就一个！！！
//注意ops就一个！！！！
int __net_init fib4_rules_init(struct net *net)
{
	int err;
	struct fib_rules_ops *ops;
	//这里就是要创建一个操作函数集，然后下面把ops会传进去
	ops = fib_rules_register(&fib4_rules_ops_template, net);
	if (IS_ERR(ops))
		return PTR_ERR(ops);
	//初始化查路由的顺序 local main default ，把规则挂到上面创建ops上（注意ops就一个）
