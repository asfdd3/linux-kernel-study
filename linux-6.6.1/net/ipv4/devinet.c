			   ...
			   int destroy, struct nlmsghdr *nlh, u32 portid)
{
	 ...
	 **/
	//如果删的是 primary 扫描后续地址，处理同子网 secondary
...
no_promotions:
	...
	*ifap = ifa1->ifa_next;
	...
	if (promote) {
		...
		//把 promote 从 secondary 区摘出来并插入 primary 区
		if (prev_prom) {
...
[ 10.0.0.1/8   primary ]
  ↓
[ 192.168.1.10/24 primary ]
  ↓
[ 192.168.1.20/24 secondary ]
  ↓
[ 192.168.1.30/24 secondary ]
  ↓
NULL
 */
//同网段的地址都被挤到 secondary 区；不在同网段的地址进入 primary 区，并按 scope 规则排好位置。
//netlink ioctl都会掉下来
			     ...
			     u32 portid, struct netlink_ext_ack *extack)
{
	...
	while (ifa1) {
		if (!(ifa1->ifa_flags & IFA_F_SECONDARY) &&
		    ifa->ifa_scope <= ifa1->ifa_scope)//主地址
		...
		if (ifa1->ifa_mask == ifa->ifa_mask &&
			...
			if (ifa1->ifa_scope != ifa->ifa_scope) {
				...
				inet_free_ifa(ifa);
				return -EINVAL;//同网段却不同 scope 会导致路由冲突
			}
			ifa->ifa_flags |= IFA_F_SECONDARY;
		}

		ifap = &ifa1->ifa_next;
		ifa1 = rtnl_dereference(*ifap);//下一个节点
	}
	...
	if (!(ifa->ifa_flags & IFA_F_SECONDARY))
		ifap = last_primary;
	//插入链表 新节点 next 指向原来的后继
	rcu_assign_pointer(ifa->ifa_next, *ifap);
	rcu_assign_pointer(*ifap, ifa);//前驱
	//插入hash表
 ...
 */
 //通过接口编号，找到这块网卡的 IPv4 地址管理结构 注意区别dev_get_by_index_rcu
...
/*把用户态发来的 RTM_NEWADDR/DELADDR 的 netlink 消息（nlmsghdr + attrs）
解析成一个in_ifaddr 对象，并返回。  */
				       ...
				       struct netlink_ext_ack *extack)
{
	...
	if (err < 0)
		goto errout;
	//取 ifaddrmsg 注意这个结构体的字段（5个固定字段之后的字段nlmsghdr）
	ifm = nlmsg_data(nlh);
	err = -EINVAL;
	//非法判断 前缀不能超过32
	...
	ifa->ifa_flags = tb[IFA_FLAGS] ? nla_get_u32(tb[IFA_FLAGS]) ://比如永久地址，一部分是用户设置一部分可能是内核设置
					 ifm->ifa_flags;
	ifa->ifa_scope = ifm->ifa_scope; //用户配置，影响路由生成通常是universal 其他的有link host 。。
	//关联in_device
	ifa->ifa_dev = in_dev;
	//用户配置的ip地址
	ifa->ifa_local = nla_get_in_addr(tb[IFA_LOCAL]);
	...
	//给这个ip地址设置地址标签，通常就是设备的名字
	if (tb[IFA_LABEL])
		nla_strscpy(ifa->ifa_label, tb[IFA_LABEL], IFNAMSIZ);//如果用户显式指定
	else
...
__be32 inet_select_addr(const struct net_device *dev, __be32 dst, int scope)
{
	...
	rcu_read_lock();
	//拿到管理ip的in_device
	in_dev = __in_dev_get_rcu(dev);//不太可能吧
	if (!in_dev)
		goto no_in_dev;
...
no_in_dev:
	...
	/* Not loopback addresses on loopback should be preferred
	   ...
	   in dev_base list.
	 */
	//这里开始遍历所有的网络设备！！！ 宁可借一个网络设备也不能没有？ 
	// 比如ip route add default via 192.168.1.1 dev eth0 但是eth0根本没有ip地址？
	for_each_netdev_rcu(net, dev) {
		//如果上面没有返回则大概率会走到这里false
...
/*在 in_dev（某个接口） 的 IPv4 地址列表里（in_ifaddr），
在满足 local / dst / scope 这些通配条件的前提下：
确认 tip(local) 是否存在 如果 local==0，就按规则选一个合适的本地地址；同时还可能要求与 dst 同子网。*/
			      ...
			      __be32 local, int scope)
{
	...
	in_dev_for_each_ifa_rcu(ifa, in_dev) {
		...
		//两个地址相同或者没传入地址则先记录一个addr
		if (!addr &&
		    (local == ifa->ifa_local || !local) &&
		    min_scope <= scope) {
			addr = ifa->ifa_local;
			if (same)//same表示同一个子网，直接退出
				break;
		}
		//没找到同一个子网的
		if (!same) {
			...
			if (same && addr) { //当 same 成立且 addr 已经有了
				if (local || !dst)
					break;
				/* Is the selected addr into dst subnet? */
				//在接口上挑一个落在 dst 子网里的本地地址
 ...
 */
//inet_confirm_addr用于在给定参数的条件下找到一个属于本机的IP地址，
// 如果没有给定具体IP地址（比如arp_ignore==1 ）.
...
void __init devinet_init(void)
{
	...
	//把 IPv4 子系统注册到 netdevice 通知链，当网卡设备状态变化（UP/DOWN/注册/注销等）时通知 IPv4 模块
	register_netdevice_notifier(&ip_netdev_notifier);

	queue_delayed_work(system_power_efficient_wq, &check_lifetime_work, 0);

	rtnl_af_register(&inet_af_ops);
	//通过注册netlink 用户态 通过netlink 来配置ip地址
