...
//如果当前 skb 不是队列里的第一片，就把它和真正的第一片交换
			      ...
			      struct sk_buff *parent)
{
	...
	int delta;
	//head是offset最小的分片，如果刚才传入的和头部不是一个，就要调整
	if (head != skb) {
		//先clone，保住他原来在树中的位置
		fp = skb_clone(skb, GFP_ATOMIC);
		if (!fp)
			return NULL;
		//把当前的给到clone之后，让fp保住信息
		FRAG_CB(fp)->next_frag = FRAG_CB(skb)->next_frag;
		if (RB_EMPTY_NODE(&skb->rbnode)) //如果当前skb不在红黑树中
			FRAG_CB(parent)->next_frag = fp;//那就把parent指针给到他
		else //在红黑树中，进行替换 队列中原来那个分片的位置交给 fp了
	 ...
	 */
	//处理skb还有fraglish的情况，通常没有吧，GRO可能会有？
	//核心思想就是把fraglist中的数据包挂到一个新申请的数据clone上面
			    ...
			    void *reasm_data, bool try_coalesce)
{
	...
	fp = FRAG_CB(head)->next_frag;//当前skb后面的skb
	rbn = rb_next(&head->rbnode);//下一个红黑树节点
	rb_erase(&head->rbnode, &q->rb_fragments);//把 head 自己从红黑树里删掉

	sum_truesize = head->truesize;
	//红黑树里还有下一个节点，当前skb里面还有数据包
	while (rbn || fp) {
		...
		while (fp) {
			...
			if (try_coalesce && skb_try_coalesce(head, fp, &stolen,
				...
				*nextp = fp; //把当前这个 fp 挂到当前位置上
				nextp = &fp->next;//下一次挂的位置
			}
			//循环用到的下一个skb
			fp = next_frag;
		}
