...
int udp_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	//corking机制检查，只有cork后才会进入这个分支，且是第二次开始，直接跳转到append data
	if (up->pending) {
		/*
	...
	//已经connect过了
		 ...
		 */
		//因为已经connect过了 下面就不用在查找路由了
		connected = 1;
	}
	//每次调进来初始化一个ipc 把地址等信息存进来
	ipcm_init_sk(&ipc, inet);
	//这里是多少 用户没设置就是0？
	ipc.gso_size = READ_ONCE(up->gso_size);
	//用户是否设置了控制信息，如果有提取到ipc中，包括ttl，tos
	...
	//如果用户send的时候没有设置ip选项，那就把setsocopt的选项（如果有）给到ipc
	if (!ipc.opt) {
	...
	////用户是否设置了路由范围 通过应该是全局可达，下面如果需要查找路由会用到
	scope = ip_sendmsg_scope(inet, &ipc, msg);
	if (scope == RT_SCOPE_LINK)
		connected = 0;
	//多播地址和单播地址 对于 出接口的处理
	if (ipv4_is_multicast(daddr)) {//目的地址是多播的情况
		if (!ipc.oif || netif_index_is_l3_master(sock_net(sk), ipc.oif))
			ipc.oif = inet->mc_index;  //这个是setsockopt设置的
		if (!saddr)
			saddr = inet->mc_addr;  //这个也是setsockopt设置的
		connected = 0;
	} else if (!ipc.oif) {//目的地址是单播的情况，并且没有通过ipc指定出接口！通常应该走这个
		ipc.oif = inet->uc_index;  //这个也是setsockopt设置的
	} else if (ipv4_is_lbcast(daddr) && inet->uc_index) {//有限广播地址的情况
	...
	//如果已经connect过了，表示已经有路由信息了，这里需要检查一下
	if (connected)
		rt = (struct rtable *)sk_dst_check(sk, 0);
	//这里注意如果没有 connect过 每次都要查找路由！！！ 如果是cork模式 则不需要每次都查找
	if (!rt) {
		struct net *net = sock_net(sk);
		__u8 flow_flags = inet_sk_flowi_flags(sk);//会影响源ip的选择
		...
		if ((rt->rt_flags & RTCF_BROADCAST) &&
			...
			goto out;
		if (connected)//如果connect过这里设置dst，后续就不会在查找了
			sk_dst_set(sk, dst_clone(&rt->dst));
	}

	if (msg->msg_flags&MSG_CONFIRM) //确认目标网络是否可达？
		goto do_confirm;
back_from_confirm:

	saddr = fl4->saddr;//下面的core会用到
	if (!ipc.addr)
		daddr = ipc.addr = fl4->daddr; //设置目的ip

	/* Lockless fast path for the non-corking case. */
	//不是cork的情况  这里直接就返回了
	if (!corkreq) {
		struct inet_cork cork;//cork 下面先初始化然后发送的时候会用到
		//这里注意传入了传输头的长度！！
