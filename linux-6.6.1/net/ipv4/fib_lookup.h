...
struct fib_alias {
	struct hlist_node	fa_list;  //多个 fib_alias 串成链表
	struct fib_info		*fa_info; //指向真正的路由数据
	dscp_t			fa_dscp;	 // ip tos ，几乎是0吧
	u8			fa_type;		 //路由类型 例如RTN_UNICAST
	u8			fa_state;		 //是否有效
	u8			fa_slen;			// 前缀长度
	u32			tb_id;         		//属于哪个路由表
	s16			fa_default;
	u8			offload;			//offload_failed
	u8			trap;				
	u8			offload_failed;
	struct rcu_head		rcu;
};
