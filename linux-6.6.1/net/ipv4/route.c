...
static struct dst_ops ipv4_dst_ops = {
	.family =		AF_INET,
	.check =		ipv4_dst_check,			//检查路由是否可以用经常被调用
	.default_advmss =	ipv4_default_advmss, //比如tcp计算mss会调用
	.mtu =			ipv4_mtu,				 //获取mtu会用到
	.cow_metrics =		ipv4_cow_metrics,	 //不会被调用
	.destroy =		ipv4_dst_destroy,		 //释放dst的时候
	.negative_advice =	ipv4_negative_advice, //tcp黑洞路由的时候会调用，是否需要丢弃这个路由
	.link_failure =		ipv4_link_failure,		//arp层出错调用
	.update_pmtu =		ip_rt_update_pmtu,		//收到icmp调用减小mtu
	.redirect =		ip_do_redirect,			 //收到icmp重定向会调用
	.local_out =		__ip_local_out,		 //发包的接口，查找路由后会设置
	.neigh_lookup =		ipv4_neigh_lookup,	 //查找arp表项
	.confirm_neigh =	ipv4_confirm_neigh,		//刷新 ARP 邻居状态
};
...
{
	struct net *net = dev_net(dst->dev);
	unsigned int header_size = sizeof(struct tcphdr) + sizeof(struct iphdr);//40
	unsigned int advmss = max_t(unsigned int, ipv4_mtu(dst) - header_size, //这里大概率是设备的mtu减去40
				    net->ipv4.ip_rt_min_advmss);  //系统默认是256
...
//把一个新建的 rtable 挂到 next hop 的缓存槽位里。
static bool rt_cache_route(struct fib_nh_common *nhc, struct rtable *rt)
{
	...
	if (prev == orig) {
		//原本的本来不为空
		if (orig) {
			//不能立刻free 比如skb还挂着他，这里要先降级
			rt_add_uncached_list(orig);
			dst_release(&orig->dst);//减少原本的缓存引用，合理！
		}
	} else {
		//撤销前面的加的引用计数
		dst_release(&rt->dst);
			   ...
			   const bool do_cache)
{
	...
	if (fi) {
		...
		//是否通过网关转发，如果是填充到rtable中 并保存网关地址
		...
		//把路由 metric 填进 ds   tcp发包很多参数都从这里拿
		...
		if (unlikely(fnhe)) //如果有 exception 大概率没有
			cached = rt_bind_exception(rt, fnhe, daddr, do_cache);
		else if (do_cache) //没有 exception，且允许缓存，就挂普通 route cache，这是最常见的场景
			cached = rt_cache_route(nhc, rt);
		if (unlikely(!cached)) { //如果缓存没成功怎么办
			/* Routes we intend to cache in nexthop exception or
			 * FIB nexthop have the DST_NOCACHE bit clear.
			 * However, if we are unsuccessful at storing this
			 * route into the cache we really need to set it.
			 */
			if (!rt->rt_gw4) { //rt 里面总得有个可以拿去做邻居解析的地址。
				rt->rt_gw_family = AF_INET;
				rt->rt_gw4 = daddr;
			}
			rt_add_uncached_list(rt); //相当于没法加到nhc上！！！那怎么办，加入 uncached list 管理！
		}
	} else
			    ...
			    bool noxfrm)
{
	...
	if (rt) {
		...
		rt->rt_pmtu = 0;
		rt->rt_mtu_locked = 0;
		rt->rt_uses_gateway = 0;//网关相关
		rt->rt_gw_family = 0;
		rt->rt_gw4 = 0;//地址

		rt->dst.output = ip_output;//设置发包接口
		if (flags & RTCF_LOCAL)
			       ...
			       struct fib_result *res)
{
	 ...
	 */
	fl4.flowi4_l3mdev = 0;
	fl4.flowi4_oif = 0;	//接收方向查路由这里设置为0
	fl4.flowi4_iif = dev->ifindex;//从哪个网口进来的
	fl4.flowi4_mark = skb->mark;//可能被netfilter 打上过标记
	fl4.flowi4_tos = tos; //查路由会用到
	fl4.flowi4_scope = RT_SCOPE_UNIVERSE;//查路由的时候指定一个大的范围
	fl4.flowi4_flags = 0;
	fl4.daddr = daddr;
	fl4.saddr = saddr;
	fl4.flowi4_uid = sock_net_uid(net, NULL);//收路径，sk设置为空
	fl4.flowi4_multipath_hash = 0;
	//对数据包parse 可能会解析出port
	...
	if (err != 0) {
		if (!IN_DEV_FORWARD(in_dev))
			err = -EHOSTUNREACH;
		goto no_route; //这里也会搞一个dst/rtable挂的skb上
	}
	//如果是广播
	if (res->type == RTN_BROADCAST) {
		//允许广播转发
		if (IN_DEV_BFORWARD(in_dev))
			goto make_route;//这里相当于仅仅转发不接收
	...
	//走到这里就是不是广播也不是本地，那就看能不能转发了
	if (!IN_DEV_FORWARD(in_dev)) {
		err = -EHOSTUNREACH;
		goto no_route;
	}
	//开了转发，但是类型不是单播，那就时异常了
...
local_input:
	...
	if (do_cache) {
		struct fib_nh_common *nhc = FIB_RES_NHC(*res);
		//从nhc 找到rtable 为什么 nhc 上会挂一个 nhc_rth_input
		//相当于从小的找到大的 因为上面res的已经限制了？
		//注意这里第一次rth可能是空下面要创建
		rth = rcu_dereference(nhc->nhc_rth_input);
		if (rt_cache_valid(rth)) {
			skb_dst_set_noref(skb, &rth->dst);//关联skb
			err = 0;
			goto out; //注意这里直接返回了
		}
	}
	//这里会设置ip_local_deliver
	...
	rth->dst.tclassid = itag;
#endif
	rth->rt_is_input = 1;//标识输入

	RT_CACHE_STAT_INC(in_slow_tot);
	//不可达的处理 input 改成 ip_error
	...
	if (do_cache) {
		...
		//尝试把这个新建好的 rtable 挂到 nhc->nhc_rth_input 上！！！
...
#ifdef CONFIG_IP_MROUTE
			||
		    (!ipv4_is_local_multicast(daddr) &&//不是224开头的特殊地址
		     IN_DEV_MFORWARD(in_dev))//开启了多播转发
#endif
		   ) {
			//多播查路由的接口
				       ...
				       unsigned int flags)
{
	...
	//非法地址检查 源地址是loopback但是非loopback发
	...
	} else if (type == RTN_MULTICAST) {//多播的情况
		flags |= RTCF_MULTICAST | RTCF_LOCAL;
		if (!ip_check_mc_rcu(in_dev, fl4->daddr, fl4->saddr, //组播地址是否是本地的
				     fl4->flowi4_proto))
			flags &= ~RTCF_LOCAL;//不是本地的
		else
			do_cache = false; //本地就不缓存
	...
	} else if ((type == RTN_LOCAL) && (orig_oif != 0) && //本地路由，但是用户指定了输出接口不缓存
	...
	fnhe = NULL;
	do_cache &= fi != NULL;//是否可以缓存 等价于  如果fi==null 那么do_cache就是false
	if (fi) { //注意单播这里大概率不是空，而组播和广播这里可能是空
		struct fib_nh_common *nhc = FIB_RES_NHC(*res);
		struct rtable __rcu **prth;
		//针对某个特定目的IP的特殊缓存 比如ICMP redirect就会有这个
		fnhe = find_exception(nhc, fl4->daddr);
		if (!do_cache) //如果已经不允许缓存，直接跳创建
			goto add;
		if (fnhe) {
			prth = &fnhe->fnhe_rth_output;
		} else { //如果这次 routing 是人为指定 next hop 而且这个 next hop 不是标准 link scope 的，那就不能缓存。
...
add:
	//这里就是申请一个rtable，设置默认的ip_output 如果是local设置input
					    ...
					    const struct sk_buff *skb)
{
	...
	if (fl4->flowi4_oif) {
		...
		if (ipv4_is_local_multicast(fl4->daddr) ||
			...
			//没有指定源ip，给个源ip，这里用的是 RT_SCOPE_LINK，因为本地多播这种流量本质上就是链路范围的。
			if (!fl4->saddr)
				fl4->saddr = inet_select_addr(dev_out, 0,
							      RT_SCOPE_LINK);
			goto make_route;//设备都拿到了直接构造路由
		}
		//没有指定源ip，但是指定了设备的情况 ，这里补上源地址
		if (!fl4->saddr) {
			//多播地址
	...
	if (res->type == RTN_LOCAL) {
		//如果没有设置源地址，就给一个源地址
		if (!fl4->saddr) {
			if (res->fi->fib_prefsrc) //首选源地址
				fl4->saddr = res->fi->fib_prefsrc;
			else
				fl4->saddr = fl4->daddr;//否则源地址就是目的地址，合理
		}
...
int __init ip_rt_init(void)
{
	...
	//初始化ip idhash表 ，分成两个部分，前半部分是id 后半部分是时间戳
	//注意这里的hash表tcp应该用不上，udp可能会用到，取决于是否connect？
	...
	//初始化每 CPU 的 rt_uncached_list，如果不缓存路由会加到这个列表中
	//什么叫不被缓存 貌似就是不被sk管理就要放到这里，应该是为了解决并发的问题？
	for_each_possible_cpu(cpu) {
		struct uncached_list *ul = &per_cpu(rt_uncached_list, cpu);
...
#endif
	//为 struct rtable 创建 slab cache
	ipv4_dst_ops.kmem_cachep =
		kmem_cache_create("ip_dst_cache", sizeof(struct rtable), 0,
				  SLAB_HWCACHE_ALIGN|SLAB_PANIC, NULL);
	//黑洞路由的复用
	ipv4_dst_blackhole_ops.kmem_cachep = ipv4_dst_ops.kmem_cachep;
	//初始化 per cpu的计数器，用于记录当前系统有多少个dst_entry
	...
	devinet_init();
	ip_fib_init();
	//注册路由统计信息的文件！/proc/net/stat/rt_cache 
	//一共三个文件第一个好像都没有了3.6内核以后就没有这个cache了
	if (ip_rt_proc_init())
