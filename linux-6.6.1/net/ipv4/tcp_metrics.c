...
void tcp_update_metrics(struct sock *sk)
{
	...
	if (icsk->icsk_backoff || !tp->srtt_us) {
		...
		if (tm && !tcp_metric_locked(tm, TCP_METRIC_RTT))
			tcp_metric_set(tm, TCP_METRIC_RTT, 0);//设置为0
		goto out_unlock;
	} else
		//获取tm结构
	...
	//丢包，等其他情况
		...
		if (!tcp_metric_locked(tm, TCP_METRIC_CWND)) {
			val = tcp_metric_get(tm, TCP_METRIC_CWND);
			//new_cwnd_metric = (old_cwnd + tp->snd_ssthresh) >> 1;
			tcp_metric_set(tm, TCP_METRIC_CWND,
				       (val + tp->snd_ssthresh) >> 1);
		}
		//更新 SSTHRESH：只在新值更大时更新
		if (!READ_ONCE(net->ipv4.sysctl_tcp_no_ssthresh_metrics_save) &&
		    !tcp_metric_locked(tm, TCP_METRIC_SSTHRESH)) {
			val = tcp_metric_get(tm, TCP_METRIC_SSTHRESH);
...
//尝试利用 历史缓存的网络性能参数（RTT、ssthresh、cwnd、乱序阈值等），来指导初始的拥塞控制和超时估算
void tcp_init_metrics(struct sock *sk)
{
	...
	if (tcp_metric_locked(tm, TCP_METRIC_CWND))
		tp->snd_cwnd_clamp = tcp_metric_get(tm, TCP_METRIC_CWND); //最大值？？

	val = READ_ONCE(net->ipv4.sysctl_tcp_no_ssthresh_metrics_save) ?//默认是1
	...
	if (val) {
		 ...
		 */
		tp->snd_ssthresh = TCP_INFINITE_SSTHRESH;  //设置慢启动阈值为全F
	}
	val = tcp_metric_get(tm, TCP_METRIC_REORDERING); //从历史连接中获取乱序阈值 默认是3
	if (val && tp->reordering != val)
		tp->reordering = val; //更新tp管理的乱序阈值上线

	crtt = tcp_metric_get(tm, TCP_METRIC_RTT); //从metric中获取缓存的rtt
	rcu_read_unlock();
reset:
