			 ...
			 unsigned int gc_cnt)
{
	...
	peer_maxttl = READ_ONCE(inet_peer_maxttl);//jiffies？
	...
	for (i = 0; i < gc_cnt; i++) {
		 ...
		 * in inet_putpeer()
		 */
		delta = (__u32)jiffies - READ_ONCE(p->dtime); //距离上次使用过了多久的时间

		if (delta < ttl || !refcount_dec_if_one(&p->refcnt))//不能删除
			gc_stack[i] = NULL;
	}
	//真正的删除
	for (i = 0; i < gc_cnt; i++) {
			       ...
			       int create)
{
	...
	//根据目的ip查找，遍历红黑树，注意第四个参数不为空则会标记为需要清除的peer
