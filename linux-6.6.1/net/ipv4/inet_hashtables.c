...
int __inet_inherit_port(const struct sock *sk, struct sock *child)
{
	...
	head = &table->bhash[bhash]; //bhash桶
	head2 = inet_bhashfn_portaddr(table, child, net, port);//bhash2的桶

	spin_lock(&head->lock);
	spin_lock(&head2->lock);
	//监听套接字的bhash桶和监听套接字的bhash2桶
	tb = inet_csk(sk)->icsk_bind_hash;
	...
	if (tb->port != port) {
		...
		goto bhash2_find;
	//这里是端口相同ip地址不同的情况进入这个分支 比如父sock监听在0.0.0.0:8080新的sock实际使用的是 192.168.1.1:8080
	//这里注意：如果新sock和listensock地址都相同那也不进入这个分支
	} else if (!inet_bind2_bucket_addr_match(tb2, child)) { 
		l3mdev = inet_sk_bound_l3mdev(sk);

bhash2_find:
		//遍历 bhash2 的桶中查找是否有一个条目可以匹配地址和ip
				...
				const int dif, const int sdif)
{
	...
	if (net_eq(sock_net(sk), net) && sk->sk_num == hnum &&
		...
		if (sk->sk_rcv_saddr != daddr)
			return -1;
		//检查接口是否匹配比如绑定的接口和接收的接口不一致就直接返回-1
		if (!inet_sk_bound_dev_eq(net, sk->sk_bound_dev_if, dif, sdif))
			return -1;
		score =  sk->sk_bound_dev_if ? 2 : 1;//如果指定了特定的接口，加2分，否则1分

		if (sk->sk_family == PF_INET)
			score++;
		//当前的cpuip和收包的cpuid在同一个加分
				...
				const int dif, const int sdif)
{
	...
	int score, hiscore = 0;
	//遍历桶下挂的每个sock，为每个套接字计算得分
	sk_nulls_for_each_rcu(sk, node, &ilb2->nulls_head) {
		//为每个套接字计算得分
		score = compute_score(sk, net, hnum, daddr, dif, sdif);
		if (score > hiscore) {
			//处理reuseport 的情况，如果开启了reuseport则优先返回reuseport的sock
			result = inet_lookup_reuseport(net, sk, skb, doff,
						       saddr, sport, daddr, hnum, inet_ehashfn);
			if (result)
				return result;
			//没开启reuseport的情况，则遍历所有的之后返回一个得分最高的
			result = sk;
			hiscore = score;
		}
	}
				    ...
				    const int dif, const int sdif)
{
	...
	//根据目的ip目的port计算一个hash值
	hash2 = ipv4_portaddr_hash(net, daddr, hnum);
	//找到对应的hash桶
	ilb2 = inet_lhash2_bucket(hashinfo, hash2);
	//注意： 这里是精确查找，区分和下面的anyaddr
...
struct sock *__inet_lookup_established(struct net *net,
				  ...
				  const int dif, const int sdif)
{
	INET_ADDR_COOKIE(acookie, saddr, daddr);//计算源ip目的ip的组合 比较key的时候会用到
	//组合源port和目的port
...
begin:
	sk_nulls_for_each_rcu(sk, node, &head->chain) {
		...
		if (likely(inet_match(net, sk, acookie, ports, dif, sdif))) {
			if (unlikely(!refcount_inc_not_zero(&sk->sk_refcnt)))//正在被释放？
				goto out;
			if (unlikely(!inet_match(net, sk, acookie, //再次比较看是否匹配
						 ports, dif, sdif))) {
				sock_gen_put(sk);
				goto begin;//再找一次
			}
...
//检查端口是否可用的核心函数，主要用于判断一个端口是否已经被 established 或 TIME_WAIT 状态的连接占用
				    ...
				    struct inet_timewait_sock **twp)
{
	...
	sk_nulls_for_each(sk2, node, &head->chain) {
		...
		if (likely(inet_match(net, sk2, acookie, ports, dif, sdif))) {
			if (sk2->sk_state == TCP_TIME_WAIT) { //如果有匹配的且是tw状态
				tw = inet_twsk(sk2);
				//这里只要系统开启可以重用tw的套接子这里大概率返回1 
				if (twsk_unique(sk, sk2, twp))
					break;//tw 类型的sock 可以重用
			}
			goto not_unique;
		}
	}
	...
	sk->sk_hash = hash;//找hahs表的hash值
	WARN_ON(!sk_unhashed(sk));
	//将新的sock插入ehahs
	__sk_nulls_add_node_rcu(sk, &head->chain);
	if (tw) {//如果是重用的那这里就要把旧的给移除
	...
	if (twp) {
		*twp = tw;
		...
		inet_twsk_deschedule_put(tw);//释放twsock的资源
	}
	return 0;
 ...
 */
//三次握手被动打开接收syn包后会把reqsock插入(后两个参数为NULL)
//三次握手接收ack的后两个参数不为空
				   ...
				   struct inet_listen_hashbucket *ilb)
{
	...
	//便利hash桶下面挂的所有socket，看是否有与当前套接字相匹配的reuseport组加入，但是必须同时满足下面一系列条件
	sk_nulls_for_each_rcu(sk2, node, &ilb->nulls_head) {
		if (sk2 != sk && //非自身
		    sk2->sk_family == sk->sk_family &&//协议族一致
		    ipv6_only_sock(sk2) == ipv6_only_sock(sk) &&
		    sk2->sk_bound_dev_if == sk->sk_bound_dev_if && //同一个网口
		    inet_csk(sk2)->icsk_bind_hash == tb && //绑定的端口相同
		    sk2->sk_reuseport && uid_eq(uid, sock_i_uid(sk2)) && //启用了reuseport
		    inet_rcv_saddr_equal(sk, sk2, false))   //地址完全相同
			return reuseport_add_sock(sk, sk2, 	//这里是将当前的sk加入到sk2中
...
int __inet_hash(struct sock *sk, struct sock *osk)
{
	...
	WARN_ON(!sk_unhashed(sk));
	//这里根据ip和端口找到listenhash桶（注意这里桶下面直接挂的是socket）
...
other_parity_scan:
	...
	for (i = 0; i < remaining; i += 2, port += 2) {
		...
		inet_bind_bucket_for_each(tb, &head->chain) {
			//端口是否匹配
			if (inet_bind_bucket_match(tb, net, port, l3mdev)) {
				//这里需要注意：如果这个条目是可以fastresues的这里直接跳过了
				//因为客户端通常要bind一个没有被使用的(或者不是建连的状态)？
				if (tb->fastreuse >= 0 ||
				    tb->fastreuseport >= 0)
					goto next_port;
				WARN_ON(hlist_empty(&tb->owners));
				//注意这里是判断是否可以重用tw的sock，如果走到了这里貌似一定会把新的sock插入到ehahs中!
...
ok:
	 ...
	 */
	//走到这里分两种情况一种是没有在bhahs中找到条目，或者是找到了可以重用的tw。
	//这里根据port和addr找到bhash2的桶
	head2 = inet_bhashfn_portaddr(hinfo, sk, net, port);
	spin_lock(&head2->lock);
	//找具体的条目，没有找到就直接创建
	tb2 = inet_bind2_bucket_find(head2, net, port, l3mdev, sk);
	...
	//判断是否加入到ehahs中过，比如说上面是有可能插入ehash
	if (sk_unhashed(sk)) {
				...
				unsigned long high_limit)
{
	...
	h->lhash2 = alloc_large_system_hash(name,
					    sizeof(*h->lhash2),
					    numentries, //建议的桶的数量
					    scale,	//21 表示每2MB分配一个桶
					    0,
					    NULL,
					    &h->lhash2_mask, //传入传出
					    low_limit,  //最大和最小限制
					    high_limit);
	init_hashinfo_lhash2(h); //初始化了每个桶的自旋锁，和链表头
