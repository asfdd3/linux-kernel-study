...
jumped:
		...
		//比较原地址目的地址 tos if等是否匹配
		if (!fib_rule_match(rule, ops, fl, flags, arg))
			continue;
		//相当于要跳转到到另一个rule去查 为什么这么设计？
		...
		} else if (rule->action == FR_ACT_NOP)//什么也不干
			continue;
		else //正常情况
			err = INDIRECT_CALL_MT(ops->action,
					       fib6_rule_action,
					       fib4_rule_action,//根据rule配置决定去查哪个表
					       rule, fl, flags, arg);
			//可能还需要过滤掉某些已经查到的结果
