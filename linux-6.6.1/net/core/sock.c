		...
		int family)
{
	...
	//从slab分配器中拿一个结构体，这个slabchace是inet_init中初始化的
	slab = prot->slab;
	if (slab != NULL) {
		sk = kmem_cache_alloc(slab, priority & ~__GFP_ZERO);
		if (!sk)
			return sk;
		//是否需要memset，可以看到这里的大小是objsize
		if (want_init_on_alloc(priority))
			sk_prot_clear_nulls(sk, prot->obj_size);
	} else
	//如果没使用slab就用kmalloc，kamlloc不也是slab吗？
...
{
	struct sock *sk;
	//调用slab 分配一个sk结构体，注意这个结构体的大小取决与prot参数的size字段
	//__GFP_ZERO表示为内存申请后需要清零的标志位
	...
	if (sk) {
		 ...
		 */
		//这里很关键，将具体协议的prot关联到了sock上
		sk->sk_prot = sk->sk_prot_creator = prot;
		//记录是否是内核创建的
		sk->sk_kern_sock = kern;
		sock_lock_init(sk);
		//如果是用户进程创建的，就增加网络命名空间的引用计数
		sk->sk_net_refcnt = kern ? 0 : 1;
		//更新当前core上所有活跃套接字的引用计数。
 ...
 */
//三次握手服务端创建完整sock的时候会调用，做的工作就是申请一个sock 并初始化字段
struct sock *sk_clone_lock(const struct sock *sk, const gfp_t priority)
{
	...
	if (!newsk)
		goto out;
	//拷贝监听套接字的部分字段(common sock 中的)到新创建的sock中(引用计数等没有被copy，拷贝的范围是有占位符号的)
	sock_copy(newsk, sk);
	//设置了tcp的ops
...
{
	u32 max_segs = 1;
	//查完路由后，已经确定了哪个网口，这里拿到驱动注册的feature
	sk->sk_route_caps = dst->dev->features;
	if (sk_is_tcp(sk)) //tcp协议默认注册GSO
		sk->sk_route_caps |= NETIF_F_GSO;
	if (sk->sk_route_caps & NETIF_F_GSO) //如果支持GSO，TCP在上一行代码已经设置了
		sk->sk_route_caps |= NETIF_F_GSO_SOFTWARE; //保底的GSO类型
	if (unlikely(sk->sk_gso_disabled)) //只有md5相关会禁用，正常不会走到这个分支
	...
	if (sk_can_gso(sk)) { 
		if (dst->header_len && !xfrm_dst_offload_ok(dst)) { 
			...
			sk->sk_gso_max_size = sk_dst_gso_max_size(sk, dst); //65536左右
			/* pairs with the WRITE_ONCE() in netif_set_gso_max_segs() */
			max_segs = max_t(u32, READ_ONCE(dst->dev->gso_max_segs), 1); //dev->gso_max_segs 这里应该是65535？
		}
	}
		     ...
		     struct sockcm_cookie *sockc)
{
	...
	case SO_MARK: //数据包设置标记（mark），用于策略路由、流量分类等
		...
		break;
	case SO_TIMESTAMPING_OLD: //时间戳控制
		if (cmsg->cmsg_len != CMSG_LEN(sizeof(u32)))
			return -EINVAL;
		...
		break;
	case SCM_TXTIME: //精确发送时间控制 指定数据包的发送时间
...
{
	if (pfrag->page) {
		if (page_ref_count(pfrag->page) == 1) { //只有自己在用
			pfrag->offset = 0; 						 //offset设置为0 
			return true;
		}
		if (pfrag->offset + sz <= pfrag->size) 			//别人在用，看空间是否够，如果够也直接返回了
			return true;
		put_page(pfrag->page);  					//不够，直接put掉
	}
	//准备分配新页
	pfrag->offset = 0; 
	//这里通常不会走把
...
int __sk_mem_raise_allocated(struct sock *sk, int size, int amt, int kind)
{
	...
	/* Under limit. */
	//小于sysctl 管理内存的的地一个阈值，这里直接返回
	if (allocated <= sk_prot_mem_limits(sk, 0)) {
		sk_leave_memory_pressure(sk); //移除内存压力状态
		return 1;
	}
	...
	//在有压力的情况下，确保每个 socket 都至少有最小的发送/接收能力
	if (kind == SK_MEM_RECV) {
		//当前套接字使用的内存是否小于4k
...
{
	amount >>= PAGE_SHIFT;
	//这里的amount的单位为页数！  这里面又减去了sk_forward_alloc 的值
...
void sock_init_data_uid(struct socket *sock, struct sock *sk, kuid_t uid)
{
	...
	sk->sk_state_change	=	sock_def_wakeup; //唤醒睡眠的进程，比如tcp状态发生改变的时候调用
	sk->sk_data_ready	=	sock_def_readable; //软中断收到数据包，唤醒睡眠的进程
	sk->sk_write_space	=	sock_def_write_space;//有写的空间，唤醒,好像几乎不会被调用
	sk->sk_error_report	=	sock_def_error_report;
	sk->sk_destruct		=	sock_def_destruct; //销毁套接字的回调

	sk->sk_frag.page	=	NULL;
	sk->sk_frag.offset	=	0;
	sk->sk_peek_off		=	-1;  //peek的偏移量

	sk->sk_peer_pid 	=	NULL; //对端的进程id，同一个主机上才有吧？
	sk->sk_peer_cred	=	NULL; //也是对端的信息
	spin_lock_init(&sk->sk_peer_lock);

	sk->sk_write_pending	=	0;  //写缓存区没有空间了
	sk->sk_rcvlowat		=	1;  //唤醒相关的水位线？1表示一个字节也唤醒
	sk->sk_rcvtimeo		=	MAX_SCHEDULE_TIMEOUT; //设置接收的超时时间 全F
	sk->sk_sndtimeo		=	MAX_SCHEDULE_TIMEOUT; ////设置发送的超时时间 全F
	...
	sk->sk_max_pacing_rate = ~0UL;  //发送速率相关，tcp拥塞控制的时候会用到bbr算法会用到
	sk->sk_pacing_rate = ~0UL;    //tcp pacing相关//全f表示不开启
	WRITE_ONCE(sk->sk_pacing_shift, 10);
	sk->sk_incoming_cpu = -1;   //记录属于哪个cpu
...
{
	//注意：这里的uid是在创建socket和inode的时候设置的
	//i_uid用于表示与该 inode 关联的文件或对象的所有者用户ID
	kuid_t uid = sock ?
