		       ...
		       struct sk_buff *skb)
{
	...
	//如果设备启用了tc
	if (dev->num_tc) {
		u8 tc = netdev_get_prio_tc_map(dev, skb->priority);//映射tc

		qoffset = sb_dev->tc_to_txq[tc].offset; //这个 TC 的 TX 队列起始编号
		qcount = sb_dev->tc_to_txq[tc].count;   //这个 TC 有多少个 TX 队列
		if (unlikely(!qcount)) {
...
struct sk_buff *validate_xmit_skb_list(struct sk_buff *skb, struct net_device *dev, bool *again)
{
	...
	//注意这里可能传入的是一个链表，循环遍历这个链表，检查是否需要GSO等。。。。
				 ...
				 struct netdev_queue *txq)
{
	...
	if (q->flags & TCQ_F_NOLOCK) {
		...
		if (q->flags & TCQ_F_CAN_BYPASS && nolock_qdisc_is_empty(q) &&
			...
			//直接发送，发送成功后继续判断是否为空，如果不为空则表示没发完 可能存在重新入队 
		...
		//bybass失败 队列不为空，没有抢到锁
		rc = dev_qdisc_enqueue(skb, q, &to_free, txq);
		//里面其实就是包装了 run_begin 如果没拿到锁就不处理，仅设置一个标志位，当解锁的时候如果发现有这个标志就重新调度tx软中断发包
		qdisc_run(q);
...
no_lock_out:
	 ...
	 */
	//如果已经在跑这个qdisc或者允许抢占的话就需要先上一个busylock
	//作用是想让当前拿到锁的cpu更容易的获取rootlock 防止被其他cpu抢走
	//造成只入队不出队列的情况
	...
	if (unlikely(test_bit(__QDISC_STATE_DEACTIVATED, &q->state))) {
		...
		rc = NET_XMIT_DROP;
	} else if ((q->flags & TCQ_F_CAN_BYPASS) && !qdisc_qlen(q) &&//快速发送路径
		   qdisc_run_begin(q)) {
		/*
		...
		//直接发送，注意这里面会解rootlock ，
		if (sch_direct_xmit(skb, q, dev, txq, root_lock, true)) {
			if (unlikely(contended)) {
				spin_unlock(&q->busylock);//这里解锁了，意味着其他线程可以入队列了
				contended = false;
			}
			__qdisc_run(q);//继续尝试处理积压的数据包
		}
		//clear bit 表示结束运行
		qdisc_run_end(q);
		rc = NET_XMIT_SUCCESS;
	} else {
		//常规入队路径，就是调用qdisc的enqueue
		rc = dev_qdisc_enqueue(skb, q, &to_free, txq);
		//尝试获取运行权，如果失败就是直接返回，因为有其他线程在处理了
...
sch_handle_egress(struct sk_buff *skb, int *ret, struct net_device *dev)
{
	...
	//常规的tc处理逻辑，这里叫mini qdisc 用户配置的 用户可以通过tc命令设置规则
	//比如五元组， mark 全匹配 最后返回处理结果，甚至可以决定进入哪个qdsic队列
	//但是如果是fifo / fq_codel / mq 等 不能决定进入哪个队列
	sch_ret = tc_run(tcx_entry(entry), skb);
egress_verdict:
	switch (sch_ret) {
	case TC_ACT_REDIRECT:
		/* No need to push/pop skb's mac_header here on egress! */
		skb_do_redirect(skb);//重定向，选择一个新的dev
		*ret = NET_XMIT_SUCCESS;
		return NULL;
	case TC_ACT_SHOT://丢弃数据包
...
{
	int tc = netdev_get_prio_tc_map(dev, skb->priority);//根据skb的优先级获取tc
	struct xps_map *map;
	int queue_index = -1;
	//检查 tc 和 tci 是否越界 nrids对于rxqs模式就是 rxqueue的数量
	if (tc >= dev_maps->num_tc || tci >= dev_maps->nr_ids)
		return queue_index;
	//拿到对应的索引
	tci *= dev_maps->num_tc;
	tci += tc;
	//这个 (tci, tc) 组合允许使用哪些 TX queues
	...
	if (map) {
		if (map->len == 1) //就一个
			...
			queue_index = map->queues[reciprocal_scale(
						skb_get_hash(skb), map->len)];//hash选一个
			 ...
			 struct sk_buff *skb)
{
	...
	if (dev_maps) {
		int tci = sk_rx_queue_get(sk);//没找到接收方向设置的地方，大概率是-1把

		if (tci >= 0)//根据tci 和tc 选一个队列
...
{
	struct sock *sk = skb->sk;
	//先取 socket 之前记住的发送队列 上次是用的哪个
	int queue_index = sk_tx_queue_get(sk);

	sb_dev = sb_dev ? : dev;
	//没初始化过或者上层容许重新选择发送队列
	if (queue_index < 0 || skb->ooo_okay ||  //注意这个ooo_okay是几乎没有流量的时候设置的
	    queue_index >= dev->real_num_tx_queues) { //队列越界
		int new_index = get_xps_queue(dev, sb_dev, skb);//通过
			//没选出来
		if (new_index < 0)
			//hash算法选一个
					 ...
					 struct net_device *sb_dev)
{
	...
	if (sender_cpu >= (u32)NR_CPUS)
		skb->sender_cpu = raw_smp_processor_id() + 1;//获取当前cpu
#endif
	//多队列的情况
	...
	return netdev_get_tx_queue(dev, queue_index);//返回对应的队列结构
}
...
//如果有egress
	...
	if (dev->flags & IFF_UP) {
		...
		if (READ_ONCE(txq->xmit_lock_owner) != cpu) {
			...
			//驱动是否返回stoped
			if (!netif_xmit_stopped(txq)) {
				dev_xmit_recursion_inc();//递归计数加1 
				skb = dev_hard_start_xmit(skb, dev, txq, &rc);//继续向下调用
...
another_round:
	...
	//处理硬件没有卸载vlan的情况 ，剥vlan头，保存到skb中
...
skip_taps://tc ingress ,和egress 基本一样
#ifdef CONFIG_NET_INGRESS
...
skip_classify:
	if (pfmemalloc && !skb_pfmemalloc_protocol(skb))
		goto drop;
//处理有vlan的情况，这里会找到vlan的netdev
	if (skb_vlan_tag_present(skb)) {
		if (pt_prev) {
			ret = deliver_skb(skb, pt_prev, orig_dev);
			pt_prev = NULL;
		}
		//这里会通过vlan id 找到对应的vlan设备然后在走一遍anotherround
		if (vlan_do_receive(&skb))
			goto another_round;
		else if (unlikely(!skb))
			goto out;
	}
	//ovs bond br 等设备在这里勾数据包
	...
	if (rx_handler) {
		...
		case RX_HANDLER_CONSUMED://已经被处理了
			ret = NET_RX_SUCCESS;
			goto out;
		case RX_HANDLER_ANOTHER://设备变了，在走一变
			goto another_round;
		case RX_HANDLER_EXACT:
			deliver_exact = true;
			break;
		case RX_HANDLER_PASS://继续处理
			break;
		default:
			BUG();
		}
...
check_vlan_id:
	...
	//没找到对应的处理函数，这里增加drop 统计信息， ifconfig 中的drop字段一部分就是在这里体现的
	//注意只是一部分
drop:
		if (!deliver_exact)
			dev_core_stats_rx_dropped_inc(skb->dev);
...
void netif_receive_skb_list_internal(struct list_head *head)
{
	struct sk_buff *skb, *next;
	struct list_head sublist;
	//在初始化一个链表，这里这么做的目的就是想去除时间戳相关的skb
	INIT_LIST_HEAD(&sublist);
	list_for_each_entry_safe(skb, next, head, list) {
		net_timestamp_check(READ_ONCE(netdev_tstamp_prequeue), skb);
		skb_list_del_init(skb);
