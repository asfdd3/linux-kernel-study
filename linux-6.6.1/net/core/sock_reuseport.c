 ...
 */
//当第一个设置了 SO_REUSEPORT的套接字成功绑定到某个特定地址和端口时，内核会通过 reuseport_alloc函数为其创建一个 struct sock_reuseport结构（即一个组），
//并将该套接字放入 socks[]数组，同时将套接字的 sk_reuseport_cb指针指向这个组
int reuseport_add_sock(struct sock *sk, struct sock *sk2, bool bind_inany)
{
	struct sock_reuseport *old_reuse, *reuse;
	//这里判断sk2是否有对应的reuseprot的组，如果没有，这里会新建一个
	if (!rcu_access_pointer(sk2->sk_reuseport_cb)) {
		//创建一个管理reuseport 的组结构，并进行初始化
	...
	old_reuse = rcu_dereference_protected(sk->sk_reuseport_cb,
					      lockdep_is_held(&reuseport_lock));
	//如果当前的套接字已经有所属的组了，并且该组中还有已经关闭的套接字，则将当前套接字加入到新组中然后直接返回了，通常不会走这个分支吧
	...
	//旧的里面还有其他套接字，不能在往下走了，不能让一个套接字属于多个组
						  ...
						  u32 hash, u16 num_socks)
{
	...
	do {
		...
		//如果端口重用组的sock已经是建立连接了那肯定就直接continue了
		if (sk->sk_state != TCP_ESTABLISHED) {
			/* Paired with WRITE_ONCE() in __reuseport_(get|put)_incoming_cpu(). */
			//如果整个端口重用组没有记录cpu则就用当前的
			if (!READ_ONCE(reuse->incoming_cpu))
				return sk;
			//如果当前的cpu和sk的cpu是同一个cpu则直接返回
				   ...
				   int hdr_len)
{
	...
	rcu_read_lock();
	//这个是listen时候更新的端口重用组，里面挂的都是reuseport的sock
 ...
 *  (or NULL on error).
 */
//服务端收到第三次握手，listensock不为listen状态了 会调用这个函数
				    ...
				    struct sk_buff *skb)
{
	...
	prog = rcu_dereference(reuse->prog);
	//是否根据bpf程序选择，如果不用bpf就使用hash
	if (!prog || prog->expected_attach_type != BPF_SK_REUSEPORT_SELECT_OR_MIGRATE) {
		//默认不启用，表示是否能将请求迁移到其他sock
		if (READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_migrate_req)) 
			goto select_by_hash;
		goto failure;
	}
...
select_by_hash:
	if (!nsk)
	//根据hash和cpu亲和性选择sock，listen的时候也调用这个
