...
static int neigh_forced_gc(struct neigh_table *tbl)
{
	...
	list_for_each_entry_safe(n, tmp, &tbl->gc_list, gc_list) {
		if (refcount_read(&n->refcnt) == 1) {//引用计数是1 相当于只有arp系统管理
			bool remove = false;

			write_lock(&n->lock);
			if ((n->nud_state == NUD_FAILED) ||  //如果已经不可达
			    (n->nud_state == NUD_NOARP) ||	 //不需要arp的表项
			    (tbl->is_multicast &&			 //多播地址对应的表项
...
static void neigh_add_timer(struct neighbour *n, unsigned long when)
{
	...
	if (time_before(n->used, n->confirmed))
		n->used = n->confirmed;
	if (unlikely(mod_timer(&n->timer, when))) { //neigh_timer_handler
	...
	for (i = 0; i < (1 << nht->hash_shift); i++) {
		...
		while ((n = rcu_dereference_protected(*np,
			...
			if (refcount_read(&n->refcnt) != 1) {
				...
				WRITE_ONCE(n->output, neigh_blackhole);//这里改成输出到黑洞
				//原来还是有效状态，就改成 NUD_NOARP；否则改成 NUD_NONE
				     ...
				     u32 flags, bool exempt_from_gc)
{
	...
	//或者超过 gc_thresh2 且距离上次清理已经超过 5 秒
						...
						unsigned long new_shift)
{
	...
	old_nht = rcu_dereference_protected(tbl->nht,
					    lockdep_is_held(&tbl->lock));
	new_nht = neigh_hash_alloc(new_shift);//新桶的数量，和初始化时候一样
	if (!new_nht)
		return old_nht;
	//重新哈希所有条目
		...
		bool exempt_from_gc, bool want_ref)
{
	...
	struct neigh_hash_table *nht;
	int error;
	//先申请一个 neighbour 结构，发包路径中这里最后一个参数是false
	...
	//arp_constructor 根据类型 初始化 neigh_ops 函数表
	...
	//当表项数超过哈希桶数量时，动态扩容为原来的 2 倍。
	if (atomic_read(&tbl->entries) > (1 << nht->hash_shift))
		nht = neigh_hash_grow(tbl, nht->hash_shift + 1);
	//计算hash值
	hash_val = tbl->hash(n->primary_key, dev, nht->hash_rnd) >> (32 - nht->hash_shift);
	//比如网络命名空间可能正在被删除 安全检查一下
	if (n->parms->dead) {
		rc = ERR_PTR(-EINVAL);
		goto out_tbl_unlock;
	}
...
static void neigh_periodic_work(struct work_struct *work)
{
	...
	//遍历桶上的条目
		while ((n = rcu_dereference_protected(*np,
			...
			//邻居条目最后一次被使用的时间在确认可达的前面，修正used时间
			if (time_before(n->used, n->confirmed) && //update的时候会设置confirmed neigh_add_timer 会设置used
			    time_is_before_eq_jiffies(n->confirmed))
				n->used = n->confirmed;
			//没人使用，标记为无效，且超过了老化时间
			if (refcount_read(&n->refcnt) == 1 &&
			    (state == NUD_FAILED ||  //手动删除或者定时器到期，或者配置了广播ip地址的时候可能会设置标志
			     !time_in_range_open(jiffies, n->used,
						 n->used + NEIGH_VAR(n->parms, GC_STALETIME)))) {
				rcu_assign_pointer(*np,
					rcu_dereference_protected(n->next,
						lockdep_is_held(&tbl->lock)));
				neigh_mark_dead(n);//标记为dead，并从gc等链表中移除
				write_unlock(&n->lock);
				neigh_cleanup_and_release(n);//清楚定时器，清除发送队列，清除相关技术
				continue;
			}
			write_unlock(&n->lock);
...
next_elt:
		 ...
		 */
		write_unlock_bh(&tbl->lock);
		cond_resched();//防止长时间占用 CPU 导致其他任务饥饿。
		write_lock_bh(&tbl->lock);
		//在短暂释放锁的窗口期内，另一个 CPU 可能对邻居表进行了扩容所以这里重新获取表
		nht = rcu_dereference_protected(tbl->nht,
						lockdep_is_held(&tbl->lock));
	}
out:
	...
	__releases(neigh->lock)
{
	...
	write_unlock(&neigh->lock);
	if (neigh->ops->solicit) //arp_solicit
...
static void neigh_timer_handler(struct timer_list *t)
{
	...
	if (state & NUD_REACHABLE) {
		//说明在 reachable_time 时间窗口内，收到过确认
		if (time_before_eq(now,
				   neigh->confirmed + neigh->parms->reachable_time)) {
			neigh_dbg(2, "neigh %p is still alive\n", neigh);
			next = neigh->confirmed + neigh->parms->reachable_time;
		//已超过 reachable_time 但当前时间 ≤ 最后一次使用时间 + 延迟探测时间
		//其实通常应该不会进入这里，这里感觉像没有立即收到回复
		...
		//上面两个条件满足，这里就是太长时间没有用了NUD_STALE
		} else {
			neigh_dbg(2, "neigh %p is suspected\n", neigh);
		...
		if (time_before_eq(now,
			...
			neigh->updated = jiffies;
			atomic_set(&neigh->probes, 0);//初始化探测次数
			notify = 1;
		       ...
		       const bool immediate_ok)
{
	...
	if (!(neigh->nud_state & (NUD_STALE | NUD_INCOMPLETE))) {
		...
		if (NEIGH_VAR(neigh->parms, MCAST_PROBES) +
			...
			//记录当前时间
			neigh->updated = now;
			//是否立即发，参数决定，工作队列貌似是不立即发
			if (!immediate_ok) {
				next = now + 1;//把arp的发送动作交给timer
			} else {
	...
	//有旧的mac的情况
		...
		neigh_dbg(2, "neigh %p is delayed\n", neigh);
		neigh_del_timer(neigh);				//删除旧的定时器
		WRITE_ONCE(neigh->nud_state, NUD_DELAY);//这里进行状态转换，表示先观察一段时间
		neigh->updated = jiffies;				//更新时间戳，记录状态变化的时间
		neigh_add_timer(neigh, jiffies +
				NEIGH_VAR(neigh->parms, DELAY_PROBE_TIME)); //重新挂一个 DELAY_PROBE_TIME 定时器
	}
	//还没有解析出skb，把skb暂时存起来，等arp完成后在发送，如果队列满了就丢弃数据包
	if (neigh->nud_state == NUD_INCOMPLETE) {
		...
		rc = 1; //这个 skb 已经被邻居子系统接管了外面不会直接发送数据包了
	}
out_unlock_bh:
	//是否立即探测
	if (immediate_probe)
		neigh_probe(neigh);//clone skb 后调用arp_solicit
	else
		write_unlock(&neigh->lock);
	local_bh_enable();
			  ...
			  struct netlink_ext_ack *extack)
{
	...
	dev    = neigh->dev;		
	old    = neigh->nud_state;//记录邻居表项原来的状态
	err    = -EPERM;
	//已经dead了 ，比如网口已经down了?
	if (neigh->dead) {
	...
	if (!(flags & NEIGH_UPDATE_F_ADMIN) &&  //如果不是管理员，相当于是人工配置的
	    (old & (NUD_NOARP | NUD_PERMANENT))) //不需要arp | 管理员手动添加的静态ARP
		goto out;
	//根据传入的flags，更新邻居条目内部的flag
	neigh_update_flags(neigh, flags, &notify, &gc_update, &managed_update);
	if (flags & (NEIGH_UPDATE_F_USE | NEIGH_UPDATE_F_MANAGED)) { //貌似只有用户配置才有这个标志
		new = old & ~NUD_PERMANENT;
		WRITE_ONCE(neigh->nud_state, new);
		err = 0;
		goto out;
	}
	//判断邻居条目是否有效，无效会进入这个分支,注意这里取决于传入的参数，通常不会进入
	if (!(new & NUD_VALID)) {
		neigh_del_timer(neigh);
		if (old & NUD_CONNECTED)//原本是可用的
			neigh_suspect(neigh);//重新设置output
		WRITE_ONCE(neigh->nud_state, new);//更新状态
		err = 0;
		notify = old & NUD_VALID; //决定是否需要通知用户空间
		if ((old & (NUD_INCOMPLETE | NUD_PROBE)) && //从等待状态变为失败
		    (new & NUD_FAILED)) {
			neigh_invalidate(neigh);//清空发送队列中的数据包
			notify = 1;
		}
		goto out;
	}
	...
	if (!dev->addr_len) { //没有mac地址的情况
		 ...
		 */
		if ((old & NUD_VALID) && //当前条目有效
		    !memcmp(lladdr, neigh->ha, dev->addr_len)) //新MAC == 旧MAC
			lladdr = neigh->ha; //重指
	} else {
		/* No address is supplied; if we know something,
		   use it, otherwise discard the request.
		 */
		//没有传入地址的情况，比如说arp_invalidate 
		err = -EINVAL;
		if (!(old & NUD_VALID)) {
	 ...
	 */
	err = 0;
	update_isrouter = flags & NEIGH_UPDATE_F_OVERRIDE_ISROUTER; //用户配置才有这个标志位
	if (old & NUD_VALID) {//当前条目有效
		//MAC不同！！！且没有OVERRIDE标志
		if (lladdr != neigh->ha && !(flags & NEIGH_UPDATE_F_OVERRIDE)) {
			...
			if ((flags & NEIGH_UPDATE_F_WEAK_OVERRIDE) &&
			    (old & NUD_CONNECTED)) {
				lladdr = neigh->ha; //// 使用旧MAC
				new = NUD_STALE;	//// 降级为STALE
			} else
				goto out;
		} else {//MAC相同，或者有OVERRIDE标志
			if (lladdr == neigh->ha && new == NUD_STALE && //第二个条件在arp_process中requset传进来会是1
			    !(flags & NEIGH_UPDATE_F_ADMIN)) //这一行大概率的情况下是1
				new = old; //保持原来的抓给你太
		}
	}
	 ...
	 */
	//如果状态或者mac地址存在变化就更新时间戳
	if (new != old || lladdr != neigh->ha)
		neigh->updated = jiffies;
	//状态改变了
	if (new != old) {
		neigh_del_timer(neigh); //删除旧定时器
		if (new & NUD_PROBE)//如果是probe状态
			atomic_set(&neigh->probes, 0);//重置探测计数器
		if (new & NUD_IN_TIMER)  //如果新状态需要定时器，就启动一个定时器
			neigh_add_timer(neigh, (jiffies +
						((new & NUD_REACHABLE) ?
						 neigh->parms->reachable_time :
						 0)));
		WRITE_ONCE(neigh->nud_state, new); //设置新的状态
		notify = 1;
	}
	//加锁后更新mac地址
	if (lladdr != neigh->ha) {
	...
	if (new == old)//状态没变
		goto out;
	if (new & NUD_CONNECTED)//进入连接状态
		neigh_connect(neigh);
	else
		neigh_suspect(neigh);//同上都是设置output回调
	if (!(old & NUD_VALID)) {//旧状态无效时
		struct sk_buff *skb;
		...
		while (neigh->nud_state & NUD_VALID &&
			 ...
			 */
			n2 = NULL;
			//这里重新查找邻居表项 为什么？因为这里的dev可能变了找到就不是原来那个邻居表项了！
...
out:
	if (update_isrouter) //通常不会进入吧
		neigh_update_is_router(neigh, flags, &notify);
	write_unlock_bh(&neigh->lock);
	if (((new ^ old) & NUD_PERMANENT) || gc_update) //NUD_PERMANENT 这个标志是否发生了变化，
		neigh_update_gc_list(neigh);
	if (managed_update)//和上面正好是相反的，比如通过ip命令配置
				 ...
				 struct net_device *dev)
{
	...
	struct neighbour *neigh = __neigh_lookup(tbl, saddr, dev,
						 lladdr || !dev->addr_len);
	if (neigh)
	//参数为邻居表项， mac地址 新的状态为未验证 ，flag表示强制覆盖
...
int neigh_resolve_output(struct neighbour *neigh, struct sk_buff *skb)
{
	...
	if (!neigh_event_send(neigh, skb)) {
		...
		struct net_device *dev = neigh->dev;
		unsigned int seq;
		//有没有cache回调，以太网设备肯定是有的，第一次没有会初始化
		if (dev->header_ops->cache && !READ_ONCE(neigh->hh.hh_len))
			//缓存目的mac和源mac
			neigh_hh_init(neigh);

		do {
			//调用netdev的headops的create memcpy二层头
	...
	skb_queue_walk_safe(&tbl->proxy_queue, skb, n) {
		...
		if (tdif <= 0) {
			...
			if (tbl->proxy_redo && netif_running(dev)) {
				rcu_read_lock();
				tbl->proxy_redo(skb);//调用发包回调 parp_redo
				rcu_read_unlock();
			} else {
				kfree_skb(skb);
			}
		    ...
		    struct sk_buff *skb)
{
	...
	NEIGH_CB(skb)->flags |= LOCALLY_ENQUEUED;//标记sbk入对

	spin_lock(&tbl->proxy_queue.lock); //删除定时器
	if (del_timer(&tbl->proxy_timer)) { //如果返回 true，说明定时器之前是被激活的，删除成功
		if (time_before(tbl->proxy_timer.expires, sched_next)) //如果旧的定时器比新包的计划时间更早，就用更早的那个时间
...
void neigh_table_init(int index, struct neigh_table *tbl)
{
	...
	INIT_LIST_HEAD(&tbl->gc_list);		//垃圾回收链表
	INIT_LIST_HEAD(&tbl->managed_list);	//受管邻居链表，由外部进程管理

	list_add(&tbl->parms.list, &tbl->parms_list);//挂到链表上
	write_pnet(&tbl->parms.net, &init_net); //设置命名空间
