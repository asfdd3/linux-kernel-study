...
struct dst_entry {
	struct net_device       *dev;    //路由关联的设备，出接口
	struct  dst_ops	        *ops;	 //操作函数集合ipv4_dst_ops
	unsigned long		_metrics;     //用户配置的
	unsigned long           expires;  //到期时间通常没有
#ifdef CONFIG_XFRM
	struct xfrm_state	*xfrm;
#else
	void			*__pad1;
#endif
	int			(*input)(struct sk_buff *); //ip_local_deliver ip_forward
	int			(*output)(struct net *net, struct sock *sk, struct sk_buff *skb); //ip_output

	unsigned short		flags;  //ipsec 策略路由等标志
...
#define DST_METADATA		0x0080
	 ...
	 */
	short			obsolete; //是否还活着
#define DST_OBSOLETE_NONE	0
#define DST_OBSOLETE_DEAD	2
#define DST_OBSOLETE_FORCE_CHK	-1
#define DST_OBSOLETE_KILL	-2
	unsigned short		header_len;	/* 额外封装头 长度more space at head required */
	unsigned short		trailer_len;	/* 尾部额外空间space to reserve at tail */
...
#endif
	int			__use;
	unsigned long		lastuse;  //最后一次使用时间 jiffes
	 ...
	 * __rcuref cache line.
	 */
	struct list_head	rt_uncached;  //下面的链表节点
	struct uncached_list	*rt_uncached_list;  //属于哪个 uncached 列表 ，不缓存的场景（不被socket管理）加入这个链表
#ifdef CONFIG_64BIT
	struct lwtunnel_state   *lwtstate;
#endif
};
...
{
	//从路由结构中获取通告mss，这里的值好象是用户配置的
	u32 advmss = dst_metric_raw(dst, RTAX_ADVMSS);

	if (!advmss)
		//调用路由的回调函数获取通告的mss //ipv4_default_advmss //这里打概率返回1460
