...
static inline void tcp_dec_quickack_mode(struct sock *sk)
{
	...
	if (icsk->icsk_ack.quick) {
		...
		if (pkts >= icsk->icsk_ack.quick) {
			icsk->icsk_ack.quick = 0; //退出快速 ACK 模式
			/* Leaving quickack mode we deflate ATO. */
			icsk->icsk_ack.ato   = TCP_ATO_MIN; //恢复延迟 ACK 的最小等待时间
		} else
			icsk->icsk_ack.quick -= pkts; //消耗一个快速ack
	}
}
...
static inline void tcp_synq_overflow(const struct sock *sk)
{
	...
	unsigned int now = jiffies; //获取当前时间
	//拿到listensock的端口重用组
	if (sk->sk_reuseport) {
	...
	//不是reuseport的情况
	last_overflow = READ_ONCE(tcp_sk(sk)->rx_opt.ts_recent_stamp);//当前队列溢出的时间
	if (!time_between32(now, last_overflow, last_overflow + HZ)) //是否在上次一处时间+1 秒的时间内，如果不再就记录这个时间
		WRITE_ONCE(tcp_sk_rw(sk)->rx_opt.ts_recent_stamp, now); //写到了接收选项中？？？
}
...
static inline int tcp_bound_to_half_wnd(struct tcp_sock *tp, int pktsize)
{
	 ...
	 */
	 //tcp的窗口大于默认mss的话就将窗口设置为一半
	if (tp->max_window > TCP_MSS_DEFAULT)
		cutoff = (tp->max_window >> 1);
	else
		cutoff = tp->max_window; //如果当前窗口很小？ 就是通告的窗口大小

	if (cutoff && pktsize > cutoff) //如果mss超过上面的得到的阈值
		return max_t(int, cutoff, 68U - tp->tcp_header_len);  //设置为阈值
	else
		return pktsize;	//通常应该走这个吧
}
...
struct tcp_skb_cb {
	...
	union {
		...
		struct {
			u16	tcp_gso_segs;
			u16	tcp_gso_size;   //一个数据包段数中的大小
		};
	};
	__u8		tcp_flags;	/* TCP header flags. (tcp[13])	*/

	__u8		sacked;		/* State flags for SACK.	*/
#define TCPCB_SACKED_ACKED	0x01	/* SKB ACK'd by a SACK block	*/ //数据包被sack确认过
#define TCPCB_SACKED_RETRANS	0x02	/* SKB retransmitted		*///重传过
#define TCPCB_LOST		0x04	/* SKB is lost			*/	//认为丢了
#define TCPCB_TAGBITS		0x07	/* All tag bits			*/
#define TCPCB_REPAIRED		0x10	/* SKB repaired (no skb_mstamp_ns)	*/
#define TCPCB_EVER_RETRANS	0x80	/* Ever retransmitted frame	*///超市重传函数中设置了
...
struct rate_sample {
	...
	u32  prior_delivered;	/* tp->delivered at "prior_mstamp" *////本次速率采样区间开始时的确认包数
...
/* control/slow paths put last */
	...
	void (*init)(struct sock *sk); //三次握手刚完成会调用
...
//计算sack和丢包的数量，其实就是在计算确认号的右侧有多少个包算异常吧!
 ...
 */
 //已经发送的 ，减去sack的和认为丢失的  在加上重传的 认为是在网络中飞行的数据包的数量 合理
 ...
 * ssthresh.
 */
//当前有效慢启动阈值(ssthresh)的函数，进入recover状态会调用 ，进入loss也会调用，其他地方也有调用
static inline __u32 tcp_current_ssthresh(const struct sock *sk)
{
	const struct tcp_sock *tp = tcp_sk(sk);
	//如果在拥塞窗口减少期间，直接返回慢启动阈值
...
static inline void tcp_slow_start_after_idle_check(struct sock *sk)
{
	...
	struct tcp_sock *tp = tcp_sk(sk);
	s32 delta;
	//默认关闭 || 正在发送， || 有自己的cong_ctrol bbr就有
	if (!READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_slow_start_after_idle) ||
	    tp->packets_out || ca_ops->cong_control)
		return;
	delta = tcp_jiffies32 - tp->lsndtime; //计算idle的时间
	if (delta > inet_csk(sk)->icsk_rto)	  //超过一个rto了
...
{
	//这里的窗口缩放因子是tcp_init_sock中设置的 ，space为rcvbuf
...
{
	//如果设置了linger2就用用户配置的，否则用系统的
	int fin_timeout = tcp_sk(sk)->linger2 ? :
		READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_fin_timeout);
	//在拿到rto
	const int rto = inet_csk(sk)->icsk_rto;
	//如果基础超时时间 fin_timeout小于3.5 * RTO 则设置为3.5个rto 
	// 大概率不会进来吧 
...
static inline bool tcp_paws_check(const struct tcp_options_received *rx_opt,
				  int paws_win)
{
	//这里一个是上一次发送数据包的时间戳（比如说这是在处理三次握手的ack则这个第一个值就是syn包的时间戳），一个是当前数据包的时间戳 paws_win为0  
				   ...
				   int rst)
{
	 ...
	 */
	//如果当前报文是一个 RST 报文，并且当前时间已经超过了外面设置的ts_recent_stamp加上 PAWS 的 MSL 时间（60秒），那么就返回 false
