...
#define ir_num			req.__req_common.skc_num   //服务端端口tcp_openreq_init会设置
#define ir_rmt_port		req.__req_common.skc_dport  //客户端端口
...
#define ireq_family		req.__req_common.skc_family
	...
	u32                     ir_mark;
	union {
		struct ip_options_rcu __rcu	*ireq_opt;  //ip选项
...
#define inet_daddr		sk.__sk_common.skc_daddr //比如tcp/udp connect 或会设置（如果udp不connect好像不会设置）
// bind中设置 //inet->inet_rcv_saddr = inet->inet_saddr = addr->sin_addr.s_addr;
#define inet_rcv_saddr		sk.__sk_common.skc_rcv_saddr  //套接字层bind的时候会设置
#define inet_dport		sk.__sk_common.skc_dport //目的port，比如建立连接的时候会设置
#define inet_num		sk.__sk_common.skc_num   //本地端口，比如bind的时候会设置

	unsigned long		inet_flags;
	//源ip地址 tcp bind中会设置 ！！！如果没有调用bind 那就会在connect中查找路由设置
	__be32			inet_saddr;    //套接字层bind会设置
	__s16			uc_ttl;  //单播的ttl，系统默认是64,可以通过setsockopt设置
	__be16			inet_sport;  //bind中设置，或者，autobind中设置
	struct ip_options_rcu __rcu	*inet_opt; //ip选项
	atomic_t		inet_id; //ip id   //发送的时候根据4元组加hash生成

	__u8			tos;   //dscp+ecn  可以通过setsockopt设置
	__u8			min_ttl;  //setsockopt设置。tcp收包的时候可以根据ttldrop掉
	__u8			mc_ttl;	//多播ttl
	__u8			pmtudisc;  //根据系统配置决定是否开启mtu探测
	__u8			rcv_tos;  //接收的tos，用户可以通过setsockopt获取
	__u8			convert_csum;
	int			uc_index;  	//输出网络设备的索引，通过setsockopt设置
	int			mc_index;	//多播，同上
	__be32			mc_addr; //发送多播的源ip地址，注意，这指的是单播地址
	struct {
		__u16 lo;
		__u16 hi;
	}			local_port_range; //本地port的范围

	struct ip_mc_socklist __rcu	*mc_list; //一个链表，管理的是当前sock加入的所有多播地址信息，以及模式
	struct inet_cork_full	cork;  //udp发包用到这个字段。存的是ip层待添字段的信息
};
