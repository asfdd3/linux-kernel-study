...
struct rtable {
	...
	int			rt_genid;          //网络命名空间相关
	unsigned int		rt_flags;  //是否是本地还是广播
	__u16			rt_type;	   //路由类型
	__u8			rt_is_input;   //标识是否是输入路由
	__u8			rt_uses_gateway; //是否通过网关

	int			rt_iif;				//输入接口 index

	u8			rt_gw_family;		//AF_INET  AF_INET6
	/* Info on neighbour */			//网关的地址
	union {
		__be32		rt_gw4;
						...
						bool *is_v6gw)
{
	...
	struct neighbour *neigh;
	//查路由的时候设置的，是否需要通过网关转发，如果需要这里daddr就是网关的ip
	if (likely(rt->rt_gw_family == AF_INET)) {
		neigh = ip_neigh_gw4(dev, rt->rt_gw4);
	} else if (rt->rt_gw_family == AF_INET6) {
		neigh = ip_neigh_gw6(dev, &rt->rt_gw6);
		*is_v6gw = true;
	//同一个子网的，这里就是用数据包的目的ip就ok
	} else {
