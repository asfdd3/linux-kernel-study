...
struct inet_peer {
	struct rb_node		rb_node;    //红黑树管理用到
	struct inetpeer_addr	daddr;  //inet_peer

	u32			metrics[RTAX_MAX];
	u32			rate_tokens;	/* rate limiting for ICMP *///icmp令牌限速
	u32			n_redirects;	//防止redirect攻击
	unsigned long		rate_last; //icpmi
	/*
	...
	union {
		struct {
			atomic_t			rid;		/* Frag reception counter *///ip分片计数！
		};
		struct rcu_head         rcu;
	};

	/* following fields might be frequently dirtied */
	__u32			dtime;	/* the time of last use of not referenced entries */ //老化时间
	refcount_t		refcnt;
};
