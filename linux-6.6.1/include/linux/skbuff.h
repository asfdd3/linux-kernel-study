...
struct sk_buff {
	...
	union {
		ktime_t		tstamp;  //ip分片会设置这个值
		u64		skb_mstamp_ns; /* earliest departure time */ //tx_delay 会设置这个值 其他地方没有了？
	};
	/*
...
void skb_flow_dissect_hash(const struct sk_buff *skb,
			   struct flow_dissector *flow_dissector,
			   void *target_container);
//对于tcp来说这里没有真正去计算hash 在建立连接的时候sk就已经去欸的那个了
...
{
	if (skb_zcopy(skb)) //零拷贝 skb 不能合并
		return false;
	if (i) {
		const skb_frag_t *frag = &skb_shinfo(skb)->frags[i - 1]; //skb的最后一个页？
		//当前写入数据使用的 page 与最后的 frag 是同一物理页
		return page == skb_frag_page(frag) &&   //判断是否是同一个页
...
{
	return ((skb->ip_summed == CHECKSUM_UNNECESSARY) || //硬件已经计算过了
		skb->csum_valid ||								//校验和正确
		(skb->ip_summed == CHECKSUM_PARTIAL &&			//硬件部分计算过
		 skb_checksum_start_offset(skb) >= 0));
}
