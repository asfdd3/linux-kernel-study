...
struct vfio_iommu {
	struct list_head	domain_list; //group attach 进来的时候，会创建或者复用 domain 挂到上
	struct list_head	iova_list;   //iova_list 用来记录 当前可用的 IOVA 地址范围。
	struct mutex		lock;
	struct rb_root		dma_list;	 //dma_list 是一棵红黑树，里面挂的是一个个 struct vfio_dma。
	...
	bool			v2;
	bool			nesting;
	bool			dirty_page_tracking; //是否开启脏页追踪。这个主要服务于虚拟机热迁移场景
	struct list_head	emulated_iommu_groups;
};

struct vfio_domain {
	struct iommu_domain	*domain;  //这个 domain 可以理解成 IOMMU 页表地址空间
...
/* 
用户态调用 `VFIO_IOMMU_MAP_DMA` 时，会传入类似这样的信息：
vaddr = 用户态虚拟地址
iova  = 希望设备看到的 DMA 地址
size  = 映射大小
flags = 允许设备读 / 写 
内核会根据这些参数创建一个 `vfio_dma`。
*/
struct vfio_dma {
	struct rb_node		node;
	dma_addr_t		iova;		/* Device address */ //这是设备 DMA 时使用的地址，也就是设备眼里的地址
	unsigned long		vaddr;		/* Process virtual addr */
	size_t			size;		/* Map size (bytes) */ //表示这段 DMA 映射的大小
	int			prot;		/* IOMMU_READ/WRITE */ //表示 IOMMU 权限，主要是
	bool			iommu_mapped;  //表示这段 vfio_dma 是否已经真正映射进 IOMMU 页表。
	bool			lock_cap;	/* capable(CAP_IPC_LOCK) */
	bool			vaddr_invalid;
	struct task_struct	*task; //这两个字段用于记录这段 DMA 映射属于哪个进程地址空间。
	struct rb_root		pfn_list;	/* Ex-user pinned pfn list */
	unsigned long		*bitmap;  //当开启脏页追踪后，每个 vfio_dma 会分配一个 bitmap，用来记录这段 DMA 区间内哪些页被认为是 dirty。
	struct mm_struct	*mm; //这两个字段用于记录这段 DMA 映射属于哪个进程地址空间。
	size_t			locked_vm;
};
...
{
	struct vfio_iommu *iommu = iommu_data; //取出当前 container 对应的 vfio_iommu .open() 返回的那个 vfio_iommu
	struct vfio_iommu_group *group;
	struct vfio_domain *domain, *d;
	bool resv_msi;
	...
	//attach group 的过程中可能需要 pin 用户页，如果当前有用户虚拟地址处于 invalid 状态，就不能继续 attach
	if (iommu->vaddr_invalid_count)
		goto out_unlock;

	/* Check for duplicates */
	ret = -EINVAL;
	//检查这个 group 是否已经 attach 过
	if (vfio_iommu_find_iommu_group(iommu, iommu_group))
		goto out_unlock;

	ret = -ENOMEM;
	//这里创建的是 VFIO type1 自己管理的 group 包装结构：
	 ...
	 */
	ret = -EIO;
	//遍历这个 iommu_group 里面的设备，拿其中一个设备作为代表设备，然后根据这个设备所属的 bus 分配一个 iommu_domain
	iommu_group_for_each_dev(iommu_group, &domain->domain,
				 vfio_iommu_domain_alloc);
	if (!domain->domain)
		goto out_free_domain;
	//主要和嵌套页表、虚拟 IOMMU 等场景相关
	if (iommu->nesting) {
		ret = iommu_enable_nesting(domain->domain);
		if (ret)
			goto out_domain;
	}
	//真正 attach group 到 IOMMU domain
	ret = iommu_attach_group(domain->domain, group->iommu_group);
	if (ret)
		goto out_domain;

	/* Get aperture info */
	//这个 IOMMU domain 能处理的 IOVA 地址范围
	geo = &domain->domain->geometry;
	/*新 attach 进来的 domain 的 IOVA 能力范围，是否会导致当前 container 已经存在的 DMA 映射变得不可用。
	如果会，就认为 aperture 冲突，拒绝 attach；如果不会，就允许后续把 container 的可用 IOVA 范围缩小到新 domain 也能支持的范围。 */
	//注意这里只是检查
	...
	//获取当前 iommu_group 对应设备的 IOMMU 保留地址区域
	ret = iommu_get_group_resv_regions(iommu_group, &group_resv_regions);
	if (ret)
		goto out_detach;
	//新 group 声明的这些保留 IOVA 范围是否已经被当前 container 中已有的 vfio_dma 映射占用了？
	 ...
	 */
	//复制当前 container 的 iova_list
	ret = vfio_iommu_iova_get_copy(iommu, &iova_copy);
	if (ret)
		goto out_detach;
	//根据新 domain 的 aperture 缩小 IOVA 范围
	ret = vfio_iommu_aper_resize(&iova_copy, geo->aperture_start,
				     geo->aperture_end);
	if (ret)
		goto out_detach;
	//从可用 IOVA 范围里排除 reserved region
	ret = vfio_iommu_resv_exclude(&iova_copy, &group_resv_regions);
	if (ret)
		goto out_detach;
	//检查是否存在 software MSI 保留区域
	resv_msi = vfio_iommu_has_sw_msi(&group_resv_regions, &resv_msi_base);
	//初始化 domain 的 group_list 把 group 挂进去
	...
	//尝试复用已有 domain
	list_for_each_entry(d, &iommu->domain_list, next) {
		if (d->domain->ops == domain->domain->ops &&  //IOMMU ops 相同
		    d->enforce_cache_coherency ==          //cache coherency 状态相同
			    domain->enforce_cache_coherency) {
			iommu_detach_group(domain->domain, group->iommu_group);
			if (!iommu_attach_group(d->domain,     //尝试 attach 到已有 domain
						group->iommu_group)) {
	...
	//把 container 里已有的 DMA 映射同步到新建的 IOMMU domain ，相当于补到新的这个iommu的页表中。
	ret = vfio_iommu_replay(iommu, domain);
	if (ret)
		goto out_detach;

	if (resv_msi) {
	//准备一份 MSI 相关的管理信息，让后续 MSI 地址能够被 IOMMU 正确处理
		ret = iommu_get_msi_cookie(domain->domain, resv_msi_base);
		if (ret && ret != -ENODEV)
			goto out_detach;
	}
	//把新 domain 加入 domain_list
...
//vfio_ioctl_set_iommu()中调用
static void *vfio_iommu_type1_open(unsigned long arg)
{
	struct vfio_iommu *iommu;
	//这里给当前 container 分配一个vfio_iommu
	...
	//初始化domain_list 和iova
	INIT_LIST_HEAD(&iommu->domain_list);
	INIT_LIST_HEAD(&iommu->iova_list);
	//用来记录用户已经映射过的 DMA 区间
	iommu->dma_list = RB_ROOT;
	//为了防止用户无限制创建 DMA 映射，内核会设置一个限制
	//后续每dma映射一次的时候会--
	iommu->dma_avail = dma_entry_limit;
	mutex_init(&iommu->lock);
	mutex_init(&iommu->device_list_lock);
	//记录当前这个 vfio_iommu 下关联的设备
...
static const struct vfio_iommu_driver_ops vfio_iommu_driver_ops_type1 = {
	...
	.owner			= THIS_MODULE,
	.open			= vfio_iommu_type1_open, 		 //用户对 container 执行 VFIO_SET_IOMMU 时调用，创建并初始化 vfio_iommu 私有结构
	.release		= vfio_iommu_type1_release,
	.ioctl			= vfio_iommu_type1_ioctl,			//处理 VFIO IOMMU 相关 ioctl，比如 DMA_MAP、DMA_UNMAP、GET_INFO 等
	.attach_group		= vfio_iommu_type1_attach_group, //将一个 VFIO group 绑定到 type1 IOMMU 后端，通常会创建或者复用 iommu_domain 并 attach 到 IOMMU group
	...
	.group_iommu_domain	= vfio_iommu_type1_group_iommu_domain, //查询某个 group 当前绑定的 iommu_domain
};
