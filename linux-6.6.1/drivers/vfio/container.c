...
int vfio_register_iommu_driver(const struct vfio_iommu_driver_ops *ops)
{
	...
	//挂到iommu_drivers_list vfio_ioctl_set_iommu会遍历这个list
					  ...
					  void *data)
{
	...
	int ret = -ENODEV;
	//遍历 container 中所有 group
	list_for_each_entry(group, &container->group_list, container_next) {
		//对每个 group 调用 IOMMU 后端的 attach_group
				 ...
				 unsigned long arg)
{
	 ...
	 */
	//container中必须已经有group，如果没有就不能设置IOMMU  || 不能重复设置iommu
	...
	list_for_each_entry(driver, &vfio.iommu_drivers_list, vfio_next) {
		...
		//open IOMMU 后端 vfio_iommu_type1_open ,这里的data就是vfio_iommu
		...
		//把 container 里的 groups attach 到这个 IOMMU 后端
		//注意这里就是调用iommu后端的attach
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case VFIO_CHECK_EXTENSION:
		...
		break;
	case VFIO_SET_IOMMU:
		//给当前VFIO container 设置一个iommu后端
		ret = vfio_ioctl_set_iommu(container, arg);
		break;
	//其他 ioctl 转发给已经绑定的 IOMMU 后端处理，比如说dma映射
	default:
		driver = container->iommu_driver;
...
static int vfio_fops_open(struct inode *inode, struct file *filep)
{
	...
	filep->private_data = container;//关联这个file
...
static const struct file_operations vfio_fops = {
	.owner		= THIS_MODULE,
	.open		= vfio_fops_open, //申请一个container
	.release	= vfio_fops_release,
	.unlocked_ioctl	= vfio_fops_unl_ioctl, //关键，设置iommu的后端，查询版本信息，以及DMA map
