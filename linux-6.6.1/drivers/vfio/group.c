...
static struct vfio {
	struct class			*class;
	struct list_head		group_list; //挂所有已经创建的组
					   ...
					   enum vfio_group_type type)
{
	...
	//后面用户态打开 /dev/vfio/17 最终就会走到这个 group 的 file operations。
		...
		enum vfio_group_type type)
{
	...
	lockdep_assert_held(&vfio.group_lock);
	//分配 VFIO 组对象 里面初始化字符设备，挂上了ops！ vfio_group_fops
	//注意这里的ops 是group 层面的ops
	group = vfio_group_alloc(iommu_group, type);
	if (IS_ERR(group))
		return group;
	// /dev/vfio/<group_id> 的名字来源
...
{
	int ret;
	//初始化group编号分配器 后续vfio_group_alloc从这里拿
	...
	/* /dev/vfio/$GROUP */
	//创建vfio class以后每创建一个group，都会创建一个device，这个device就是类被就是vfio
	vfio.class = class_create("vfio");
	if (IS_ERR(vfio.class)) {
		ret = PTR_ERR(vfio.class);
		goto err_group_class;
	}
	//设置回调 设备节点的路径，创建group的时候用到
	vfio.class->devnode = vfio_devnode;
	//申请group字符设备号的范围，后面申请group的时候使用
